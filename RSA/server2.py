...
def generate_key_pair(bits=2048):
    ...
    # Create keys manually without CRT parameters
...
def write_to_csv(data, filename='message_logRSA.csv'):
    with open(filename, mode='a', newline='') as file:
        writer = csv.writer(file)
        if file.tell() == 0:
            writer.writerow(["User", "Message", "Decryption Time (s)", 
                             "Encryption Time (s)", "Ciphertext"])
